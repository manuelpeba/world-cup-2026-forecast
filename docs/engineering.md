...
This project implements a production-style football forecasting pipeline
that combines:
...
The system simulates complete international tournaments (currently a
32-team World Cup format) and estimates advancement probabilities for
each team.
...
This document describes the engineering architecture of the forecasting
system, including:
...
The objective is to explain how the different modules interact to
produce scalable tournament forecasts.

# System Architecture

The simulation pipeline is divided into several independent modules.

data → features → match prediction → tournament simulation → aggregation
→ reporting → dashboard
...
quarterfinalists semifinalists finalists champion
...
simulate_group_stage() simulate_knockout_stage() assemble
TournamentRunResult
...
team_probabilities champion_distribution stage_presence stage_counts

Probabilities are computed by averaging binary stage indicators across
simulations.
...
team_probabilities.parquet team_probabilities.csv

champion_distribution.parquet champion_distribution.csv
...
100,000 tournament simulations executed in seconds to minutes
...
simulation_id group_tables group_stage_results qualified_teams
quarterfinalists semifinalists finalists champion match_results metadata
...
A Streamlit dashboard (`app/streamlit_app.py`) provides an interactive
interface.
...
32 teams 8 groups 2 teams advancing per group standard knockout bracket

The real 2026 World Cup format will include:

48 teams 12 groups best third-place teams advancing round of 32
...
The project demonstrates how to build a modular football forecasting
